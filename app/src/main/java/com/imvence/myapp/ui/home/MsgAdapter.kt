...
//    }
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MsgAdapter.InnerHolder {
        val itemView: View = LayoutInflater.from(context).inflate(R.layout.msg_item,parent,false)
        return InnerHolder(itemView)
    }
     ...
     * 给view绑定内容
     */
    override fun onBindViewHolder(holder: MsgAdapter.InnerHolder, position: Int) {
        holder.msg_nickname.text = datas[position].nickname
        holder.msg_day.text = datas[position].day
        holder.msg_content.text = datas[position].content
        holder.msg_avatar.setImageURI(datas[position].avatar)
    }
